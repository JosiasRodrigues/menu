...
cardapio.metodos = {
    ...
    verMais: () => {
        ...
        const ativo = $(".container-menu a.active").attr('id').split('menu-')[1]; //menu-burgers
    ...
    adicionarAoCarrinho: (id) => {
        ...
        if (qntdAtual > 0) {
           ...
           const item = $.grep(filtro, (e, i) => { return e.id == id });

           if (item.length > 0) {

            //validar se ja existe este item no carrinho

            let existe = $.grep(MEU_CARRINHO, (elem, index) => { return elem.id == id });

            //caso ja exista o item no carringo, so altera a quantidade
            if (existe.length > 0) {
                const objIndex = MEU_CARRINHO.findIndex((obj => obj.id == id));
    ...
    //atualizar o badge de totais dos botoes meu carinho
    atualizarBadgeTotal: () => {
    ...
    carregarCarrinho: () => {
        ...
        if (MEU_CARRINHO.length > 0) {
            ...
            $("#itensCarrinho").html('<p class="carrinho-vazio"><i class="fa fa-shopping-bag"></i> Seu carrinho está vázio.</p>');
            cardapio.metodos.carregarValores();
        }
    ...
    //aumentar quantidade do intem no carrinho
    ...
    //carrega os valores de subtota, entrega e total.
    carregarValores: () => {
...
cardapio.templates = {
    ...
    item: `
    <div class="col-12 col-lg-3 col-md-3 col-sm-6 mb-5 fadeInUp">
